...
def train_net(net, device, epochs=5, batch_size=64, lr=0.001, nifti_train=-1,
              ...
              save_cp=True, load_layer=[], model_mark='test',dcwt=0.8):
    ...
    all_loader = torch.utils.data.ConcatDataset([train, val, test])  # 1

    train_loader = DataLoader(all_loader, batch_size=batch_size, shuffle=False, #5
    ...
    optimizer = torch.optim.Adam([{'params': filter(lambda p: id(p) in load_params and p.requires_grad, net.parameters()),
                                   'lr': 0.25*lr},
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            intensity = [0,100,200,255]
            true_mask_new = torch.zeros((true_masks.shape[0], true_masks.shape[2], true_masks.shape[3]))
            for i in range(len(intensity)):
                true_mask_forClass = true_masks[:, i, :]
                true_mask_new[np.where(true_mask_forClass == 1)] = intensity[i]
            true_mask_new.unsqueeze(1)
            j += 1
            save_image(imgs, 'results/result_epoch%03d_fig%03d.png' % (epoch, j))
            save_image(true_mask_new.unsqueeze(1), 'results/mask_result_epoch%03d_fig%03d.png' % (epoch, j))



    #         masks_pred = net(imgs)
    #         loss1 = criterion1(masks_pred.view(-1), true_masks.view(-1))
    #         loss2 = criterion2(masks_pred, true_masks, weights=[0, 0.45, 0.1, 0.45])
    ...
    # logging.info(f'Training Finished, the best validation loss is %.4f in epoch %d!' % (
    #     sorted_val_score[0][1], sorted_val_score[0][0]))
    #
    # # test
    # for best_epoch, _ in sorted_val_score:
...
def get_args():
    ...
    parser.add_argument('-e', '--epochs', metavar='E', type=int, default=1, #4
                        help='Number of epochs', dest='epochs')
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=36, #3
