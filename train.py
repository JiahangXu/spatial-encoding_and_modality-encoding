...
def train_net(netG, device, epochs=5, batch_size=128, lr=0.001, ntrain=-1,
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            lossG_l2.backward()
            optimizer_G.step()
            logging.info('[%03d/%03d][%03d/%03d] Generator MSE: %.4f; [%s]' % (
                epoch, epochs, i, len(train_loader), lossG_l2.item(), task_mode[0]))

            global_step += 1

        # validation after the epoch finished
        val_l2 = eval_net(netG, val_loader, device, n_val, epoch, model_mark)
...
def eval_net(netG, loader, device, n_val, epoch, model_mark):
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            if i % 50 == 0 and epoch > 100:
                save_generate_image(img_real, img_tasks, task_mask, img_fake,
